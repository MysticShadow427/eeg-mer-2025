...
class EremusDataset(Dataset):
    def __init__(self, subdir, split_dir, split="train", task="emotion_recognition", ext="npy", transform=None, prefix="",image_transform=False):
        ...
        self.split = split
        self.label_name_sub = "subject_id"
        self.label_name_emo = "label"
        self.ext = ext
    ...
    def __getitem__(self, idx):
        ...
        sample = {
            ...
            "label_emo": sample[self.label_name_emo] if "test" not in self.split else -1,
            "label_sub": sample[self.label_name_sub] if "test" not in self.split else -1,
        }
        if self.transform:
