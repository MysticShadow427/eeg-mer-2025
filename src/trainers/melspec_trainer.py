...
from src.utils import EEGToMelSpectrogram,FocalLoss# augment_features
from src.augmentations import gaussian_noise, ft_surrogate
import torch.nn.functional as F
from torch.nn.utils.rnn import pad_sequence

# we need to add augmentation functions,pass that to model,detach that tensors and then make these as n_views to pass to supcon losss,thats the easiest i see
class Trainer:
    ...
    def __init__(self, args):
        ...
        # self.contrastive_criterion = MultiPosConLoss()
    ...
    def windowing(self, batch):
        ...
        mel_specs = []
        for chunk in chunks:
            chunk = chunk.unsqueeze(0)  # Add batch dimension for mel transform
            mel_spectrogram = self.mel_transform.eeg_to_mel(chunk)  # Convert EEG to mel-spectrogram
            # mel_spectrogram = F.interpolate(mel_spectrogram, size=(128, 80), mode='bilinear', align_corners=False)
            mel_spectrogram = mel_spectrogram.squeeze(0).transpose(0, 1)  # Remove batch dimension
            mel_specs.append(mel_spectrogram)
        
        # Concatenate all mel-spectrograms and add them to the batch
        batch['mel_spec'] = torch.stack(mel_specs)
            ...
            for epoch in range(self.args.epochs):
                ...
                for split in splits:
                    ...
                    for batch in tqdm(loaders[split]):
                        ...
                        # Forward
                        mels = [item['mel_spec'] for item in batch]  # Gather mel specs from each item
                        mels = pad_sequence(mels, batch_first=True).to(self.args.device)
                        outputs = net(mels)
    ...
    def test(self, test_loaders):
        ...
        for split, test_loader in test_loaders.items():
            ...
            for batch in tqdm(test_loader):
                ...
                inputs = inputs.to(self.args.device)
                # mels = mels.to(self.args.device)
                mels = [item['mel_spec'].transpose(0, 1) for item in batch]  # Gather mel specs from each item
                mels = pad_sequence(mels, batch_first=True).to(self.args.device)
