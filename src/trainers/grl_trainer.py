...
from src.utils import mmd_loss,orthogonality_loss,EEGToMelSpectrogram, FocalLoss,compute_mi_cfm,compute_pcc_cfm,compute_cross_entropy,compute_plv_cfm,compute_te_cfm,fuse_cfms # augment_features
...
class Trainer:
    ...
    def __init__(self, args):
        ...
        self.args = args
        # Create criterion
        self.criterion_emo = nn.CrossEntropyLoss()
        self.criterion_sub = nn.CrossEntropyLoss()
        self.ortho_loss = orthogonality_loss()
            ...
            for epoch in range(self.args.epochs):
                ...
                for split in splits:
                    ...
                    for batch in tqdm(loaders[split]):
                        ...
                        # cfms = batch['cfm']
                        # mels = batch['mel_spec']
                        labels_emo = batch['label_emo']
                        labels_sub = batch['label_sub']
                        ...
                        labels_emo = labels_emo.to(self.args.device)
                        labels_sub = labels_sub.to(self.args.device)
                        ...
                        # Forward
                        # print(cfms.shape)
                        outputs_emo, outputs_sub, feats_emo, feats_sub = net(inputs)
                        mmd_loss_value = 0.0
                        unique_emotions = torch.unique(labels_emo)
                        for emotion in unique_emotions:
                            emotion_mask = labels_emo == emotion
                            for subject_1 in torch.unique(labels_sub):
                                for subject_2 in torch.unique(labels_sub):
                                    if subject_1 != subject_2:
                                        _,_,_,subject_1_features = net(inputs[emotion_mask & (labels_sub == subject_1)])
                                        _,_,_,subject_2_features = net(inputs[emotion_mask & (labels_sub == subject_2)])
                                        if subject_1_features.size(0) > 1 and subject_2_features.size(0) > 1:
                                            mmd_loss_value += mmd_loss(subject_1_features, subject_2_features)
                        ...
                        # Check NaN
                        if torch.isnan(outputs_emo).any():
                            raise FloatingPointError('Found NaN values')
                        if torch.isnan(outputs_sub).any():
                            raise FloatingPointError('Found NaN values')
                        if torch.isnan(feats_emo).any():
                            raise FloatingPointError('Found NaN values')
                        if torch.isnan(feats_sub).any():
                            raise FloatingPointError('Found NaN values')
                        ...
                        # Compute loss
                        loss_emo = self.criterion_emo(outputs_emo, labels_emo)
                        loss_sub = self.criterion_sub(outputs_sub,labels_sub)
                        # do we need to normalize is the main question
                        loss_ortho = self.ortho_loss(feats_emo,feats_sub)
                        loss = loss_emo - 0.7* loss_sub + loss_ortho + mmd_loss_value
                        ...
                        if self.args.use_voting and split != 'train':
                            if self.args.voting_strategy == 'mean':
                                outputs_emo = outputs_emo.mean(dim=0)
                            elif self.args.voting_strategy == 'max':
                                outputs_emo, _ = outputs_emo.max(dim=0)
                            elif self.args.voting_strategy == 'min':
                                outputs_emo, _ = outputs_emo.min(dim=0)
                            elif self.args.voting_strategy == 'median':
                                outputs_emo, _ = outputs_emo.median(dim=0)
                            elif self.args.voting_strategy == 'majority':
                                try:
                                    outputs_emo = outputs_emo.argmax(dim=1).mode().values[0]
                                except IndexError:
                                    outputs_emo = outputs_emo.argmax(dim=1).mode().values
                            else:
                                raise ValueError(f"Voting strategy {self.args.voting_strategy} not recognized")
                            
                            outputs_emo = outputs_emo.unsqueeze(0)
                            labels_emo = labels_emo[0].unsqueeze(0)

                        epoch_labels.append(labels_emo)
                        epoch_outputs.append(outputs_emo)
    ...
    def test(self, test_loaders):
        ...
        for split, test_loader in test_loaders.items():
            ...
            for batch in tqdm(test_loader):
                ...
                # Forward
                outputs_emo, outputs_sub,_,_ = net(inputs)
                ...
                # Predictions
                if self.args.voting_strategy == 'mean':
                    prediction = outputs_emo.mean(dim=0).argmax()
                elif self.args.voting_strategy == 'max':
                    prediction, _ = outputs_emo.max(dim=0)
                    prediction = prediction.argmax()
                elif self.args.voting_strategy == 'min':
                    prediction, _ = outputs_emo.min(dim=0)
                    prediction = prediction.argmax()
                elif self.args.voting_strategy == 'median':
                    prediction, _ = outputs_emo.median(dim=0)
                    prediction = prediction.argmax()
                elif self.args.voting_strategy == 'majority':
                    try:
                        prediction = outputs_emo.argmax(dim=1).mode().values[0]
                    except IndexError:
                        prediction = outputs_emo.argmax(dim=1).mode().values
                else:
                    raise ValueError(f"Voting strategy {self.args.voting_strategy} not recognized")
