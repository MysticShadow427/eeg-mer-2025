...
def parse():
    ...
    parser.add_argument('--data_type', type=str, default="preprocessed", choices=["raw", "pruned", "preprocessed", "cfm"])
    ...
    ## EEGConformer and EEGTransformer
...
def main(args):
    ...
    def train_func():
        ...
        if args.sweep:
            ...
            # Init run
            run = wandb.init(project=args.task)
        ...
        if updated_args.debug:
            run = wandb.init(project=updated_args.task, mode="disabled")
        elif not updated_args.sweep:
            run = wandb.init(project=updated_args.task)
    ...
    if args.sweep:
        # Run sweep
        wandb.agent(sweep_id, function=train_func, project=args.task)
    else:
        train_func()
